...
from peft import get_peft_config, get_peft_model, PromptTuningInit, PromptTuningConfig, TaskType, PeftType,LoraConfig,AdaptionPromptConfig
import logging
...
class BrainGPT(nn.Module):
    def __init__(self,config,lora_config,use_lora):
    ...
    def load_decoder_checkpoint(self,path):
        ...
        self.decoder.from_pretrained(path)
        self.llm = self.decoder.model
...
if __name__ == '__main__':
    ...
    model = BrainGPT(model_config,lora_config,use_lora = train_config['use_lora'])
    
    param_list = []
    if train_config['lr_linear'] > 0:
        param_list.append({'params':model.linear.parameters(),'lr':train_config['lr_linear']})
    else:
        for param in model.linear.parameters():
            param.requires_grad = False
    
    if train_config['lr_encoder'] > 0:
        param_list.append({'params':model.encoder.parameters(),'lr':train_config['lr_encoder']})
    else:
        for param in model.encoder.parameters():
            param.requires_grad = False
    param_list.append({'params':model.llm.parameters(),'lr':train_config['lr_decoder']})
    

    optimizer = AdamW(param_list,eps = train_config['eps'])
    ...
    #scheduler = CosineAnnealingLR(optimizer,T_max = num_steps*args.num_epochs)
    ...
    model.load_encoder_checkpoint(checkpoint_path + '/stage1')
    ...
    if train_config['save_checkpoint']:
        ...
        with open(checkpoint_path + '/train_config.json','wt') as f:
        ...
        if (epoch) % epochs_per_eval == 0:
            ...
            if val_mean_wer < best_val_wer:
                ...
                if train_config['save_checkpoint']:
                    ...
                    model.save_decoder_checkpoint(checkpoint_path)
            else:
                count += 1
